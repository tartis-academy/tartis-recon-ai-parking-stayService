...
class ListStaysUseCaseTest {
    ...
    @BeforeEach
    void setUp() {
        useCase = new ListStaysUseCase(stayPersistence, new StayDTOFactory());
    }
    ...
    @Test
    @DisplayName("Debe mapear la pagina de dominio a StayPageDTO conservando los metadatos")
    void shouldReturnMappedPage() {
        UUID stayId = UUID.randomUUID();
        when(stayPersistence.findPage(StayStatus.IN_PROGRESS, 0, 20))
                .thenReturn(new StayPersistence.StayPage(List.of(sampleStay(stayId)), 0, 20, 1L, 1));
